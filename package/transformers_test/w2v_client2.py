...
if __name__ == "__main__":
    ...
    model = Word2Vec(
        sentences, vector_size=200, sg=0, window=5, min_count=1, workers=4, epochs=5
    )
    ...
    s1_afv = avg_feature_vector(
        "游戏异常", model=model, num_features=200, index2word_set=index2word_set
    )
    s2_afv = avg_feature_vector(
        "游戏画面", model=model, num_features=200, index2word_set=index2word_set
    )
