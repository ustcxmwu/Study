...
class TRPO(object):
    ...
    def hessian_matrix_vector_product(self, states, old_action_dists, vector):
        ...
        kg_grad_vector = torch.cat([grad.view(-1) for grad in kl_grad])
        kl_grad_vector_product = torch.dot(kg_grad_vector, vector)
...
def main():
    ...
    torch.manual_seed(0)
    agent = TRPO(
        hidden_dim, env.observation_space, env.action_space, lmbda, kl_constraint, alpha, critic_lr, gamma, device
    )
