...
class ImageScrapy(object):
    ...
    def get_html_title(self, soup, img_num: int):
        return (
            re.split(r"[\b\W\b]+", soup.find("title").text)[0]
            + "["
            + str(img_num)
            + "P]"
        )
    ...
    def download_img(
        self, selector: Callable[[bs4.Tag], bool], img_link_attr: str = "src"
    ):
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Dropdown(
            ["xyz", "xp1024"], label="Selector", info="Will add more selectors later!"
        ),
    ],
